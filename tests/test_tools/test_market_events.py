...
@pytest.mark.asyncio
class TestMarketEventsTool:
    def test_valid_operations(self):
        expected = {"get_market_holidays", "get_upgrade_downgrade", "get_merger_acquisition"}
        assert expected == MarketEventsTool.VALID_OPERATIONS

    async def test_validate_operation_invalid(self, test_config: AppConfig):
        async with FinnhubClient(test_config) as client:
            tool = MarketEventsTool(client)
